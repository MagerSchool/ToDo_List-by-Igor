const fetchTask = async () => {
    try {
        const todos = await fetch('https://jsonplaceholder.typicode.com/todos?_start=0&_limit=5');
...
export const renderTask = async (taskBody, taskStub) => {
    ...
    numberOfTasksEl.innerHTML = tasks.length;

    const numberOfDoneTasksEl = document.querySelector('#done_task');
    numberOfDoneTasksEl.innerHTML = tasks.filter(task => task.completed).length;
        ...
        <div id="task-${id}" class="todo_list-item ${ completed ? 'todo_list-item--done' : '' }">
            ...
            <div class="todo_list_button-wrap">
                <button class="todo_list-done" data-action="done">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" data-action="done">
                ...
                <button class="todo_list-remove" data-action="remove">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" data-action="remove">
